...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.dp4 = nn.Dropout(0.15)
        self.fc5 = nn.Linear(128, 256)
        self.bn5 = nn.BatchNorm1d(256)
        self.dp5 = nn.Dropout(0.15)
...
def minus2(y):
    ...
    l = int(y.shape[0] / 2)
    for i in range(l):
        minus.append(dis2me1(y[i, 1:], y[i + l, 1:]))
    ...
    for epoch in range(num_epochs):
        with tqdm(total=num_batches, desc=f"Epoch {epoch + 1}/{num_epochs}") as pbar:
            ...
            for X, y in train_iter:
                ...
                l1 = loss(out[:half], y[:half])
                l2 = loss(out[half:], y[half:])
...
k, num_epochs, lr, weight_decay, batch_size = 5, 800, 0.000250, 0.0000025, 64
...
def train_and_pred(train_features, test_features, train_labels, test_labels,
    ...
    df_.to_csv('submission1.csv', index=False)
