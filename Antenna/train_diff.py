...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.p = PositionalEncoding(16, 0)
...
def k_fold(k, X_train, y_train, num_epochs, learning_rate, weight_decay,
    ...
    for i in range(k):
        ...
        if i >= 0:
            d2l.plot((range(1, num_epochs + 1)), [train_ls, valid_ls],
...
# p1 = torch.tensor([[2.9976248741149902, 3.97798228263855, 0.43577873706817627]]).to(device)
# p2 = torch.tensor([[-2.616975784301758, 4.18803596496582, 0.7821723818778992]]).to(device)
# p3 = torch.tensor([[-0.25687047839164734, -4.90140962600708, 0.9540449380874634]]).to(device)
loss = nn.MSELoss()
...
diff_labels1 = train_labels[::2, 1:] - train_labels[1::2, 1:]
diff_dist1 = torch.norm(diff_labels1, dim=1)
length = int(train_labels.shape[0]/2)
diff_labels2 = train_labels[:length, 1:] - train_labels[length:, 1:]
diff_dist2 = torch.norm(diff_labels2, dim=1)
diff_dist = torch.cat((diff_dist1, diff_dist2), dim=0).reshape(-1, 1)
diff_fea1 = train_features[::2, 1:] - train_features[1::2, 1:]
diff_diff1 = torch.norm(diff_fea1, dim=1)
diff_fea2 = train_features[:length, 1:] - train_features[length:, 1:]
diff_diff2 = torch.norm(diff_fea2, dim=1)
diff_diff = torch.cat((diff_diff1, diff_diff2), dim=0).reshape(-1, 1)
# train_min = 1e31
# for lr in range(1, 1000):
k, num_epochs, lr, weight_decay, batch_size = 5, 800, 0.00020, 0.000005, 64
...
def train_and_pred(train_features, test_features, train_labels, test_labels,
    ...
    #     print('  value:', param.data)
        ...
        df_.to_csv('submission_diff.csv', index=False)
...
# ll = dff['s'].tolist()
# lll = df['s'].tolist()
# llll = [a - b for a,b in zip(ll, lll)]
# print(ll)
# preds = pd.read_csv('./submission_s1.csv')
